...
export default function LeftSensorsOverlay() {
  ...
  const acc = sensorsData?.mpu.acc ?? [0, 0, 0];
  const gyro = sensorsData?.mpu.gyro ?? [0, 0, 0];
  const angle = sensorsData?.mpu.angle ?? [0, 0, 0];
...
{/* mpu */}
      <div className="space-y-1.5 bg-black/40 backdrop-blur-md p-3 rounded-2xl border border-white/5 w-52 shadow-2xl">
        ...
        <div className="pt-2 border-t border-white/10 flex justify-between items-center">
          <span className="text-gray-400 font-bold text-[10px] uppercase tracking-tighter">Temperature</span>
          <span className="text-cyan-400 font-black">{tempIn.toFixed(2)} °C</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col gap-1">
      ...
      <div className="flex gap-4 text-xs font-mono text-white/80">
        {values.map((v: number, i: number) => (
          <span key={i}>
            <span className="text-cyan-600 font-bold">{labels[i]}:</span> {v.toFixed(2)}
          </span>
        ))}
      </div>
    </div>
